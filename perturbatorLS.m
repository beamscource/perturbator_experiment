function [subjectFolder, gender] = perturbatorLS(subjectID, gender, loops)
	...
	experimentFolder = 'D:\perturbator\learnsound';
	data  = 'data';
	...
	for file = 1:last
		...
		if strcmp(phase, 'soundcheck')
			...
			if ~exist('language', 'var')
				language = input('Participant''s language (g): ','s');
			end
		...
		% provide the path to the target sound...
		if strcmp(phase, '2_training') || strcmp(phase, '4_training')
			stimuli = 'targetsound';
			stimuliFolder = fullfile(experimentFolder, stimuli);
			S.audapterplay = fullfile(stimuliFolder);
			% see promter_ini_base lines 856 ff
		end

		% ...to the noise sound
		if strcmp(phase, '3_test') || strcmp(phase, '5_test')
