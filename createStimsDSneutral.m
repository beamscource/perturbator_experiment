function createStimsDSneutral(subjectFolder, gender, formants, pertSet, phase)
	...
	if strcmp(phase, 'baseline')
		...
		txtStruct{4} = 'ми';
		txtStruct{5} = ['mi_1_0000_01_REP#fb 1#pertAmp 0#pertPhi pi*0#fn1 ' int2str(formants.iF1) '#fn2 ' int2str(formants.iF2) '%no perturbation'];
		txtStruct{6} = 'мы';
		txtStruct{7} = ['my_1_0000_02_REP#fb 1#pertAmp 0#pertPhi pi*0#fn1 ' int2str(formants.yF1) '#fn2 ' int2str(formants.yF2) '%no perturbation'];
		txtStruct{8} = 'бы';
		txtStruct{9} = ['by_1_0000_03_REP#fb 1#pertAmp 0#pertPhi pi*0#fn1 ' int2str(formants.yF1) '#fn2 ' int2str(formants.yF2) '%no perturbation'];
		txtStruct{10} = 'бу';
		txtStruct{11} = ['bu_1_0000_04_REP#fb 1#pertAmp 0#pertPhi pi*0#fn1 ' int2str(formants.uF1) '#fn2 ' int2str(formants.uF2) '%no perturbation'];
		txtStruct = txtStruct';
		...
		for i = 1:2:length(fields)
			...
			txtStruct{3} = ['Aufgabenbeschreibung ' phase];
			txtStruct{4} = 'мы';
			txtStruct{5} = ['my_1_0000_01_REP#fb 1' pertSettingsA1 '#fn1 ' int2str(formants.iF1) '#fn2 ' int2str(formants.iF2) pertSettingsA2]; 
			txtStruct{6} = 'бы';
			txtStruct{7} = ['by_1_0000_02_REP#fb 1' pertSettingsB1  '#fn1 ' int2str(formants.uF1) '#fn2 ' int2str(formants.uF2) pertSettingsB2];
			txtStruct = txtStruct';
